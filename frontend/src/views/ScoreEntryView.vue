...
async function initScores() {
  ...
  // Target the active round, or fallback to the first round if nothing is active
...
const currentRound = computed(() => league.activeRound || (league.rounds.length > 0 ? league.rounds[0] : null))

const completedCount = computed(() => league.matches.filter(m => m.status === 'Completed' || m.status === 'Forfeited').length)
...
<template>
  <div class="max-w-3xl mx-auto px-4 py-6 space-y-4 animate-fade-in pb-20">
      ...
      <div>
        ...
        <p class="text-[10px] font-bold uppercase tracking-widest mt-2" style="color: var(--text-muted);">
          <template v-if="currentRound">
            {{ t('matches.round', { num: currentRound.round_number }) }} • {{ getSeasonLabel(league.selectedSeason) }}
          </template>
        </p>
      </div>
    ...
    <!-- Main List -->
    <div class="space-y-2">
      <div v-if="!currentRound && !league.loading" class="card p-10 flex flex-col items-center text-center gap-4">
