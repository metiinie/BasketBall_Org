...
async function initData() {
  ...
  // Use active round if available, otherwise fallback to the first round in the list
  const targetRound = league.activeRound || (league.rounds.length > 0 ? league.rounds[0] : null)
  
  if (targetRound) {
    editForm.value.start_date = targetRound.start_date?.split('T')[0] || ''
    editForm.value.end_date = targetRound.end_date?.split('T')[0] || ''
    await Promise.all([
      league.fetchTeams(league.selectedGender),
      league.fetchMatches(targetRound.id),
    ])
  } else {
    // If no rounds exist at all, still fetch teams for the current gender to show empty states correctly
