...
async function initDashboard() {
  ...
  // Use active round if available, otherwise fallback to the first round found
  ...
  if (targetRound) {
    ...
    // If no rounds exist, still fetch teams and clear old matches
...
const completedMatches = computed(() => league.matches.filter(m => m.status === 'Completed' || m.status === 'Forfeited').length)
...
const top3Standings = computed(() => league.standings.slice(0, 3))
